...
def replace_weird_characters_with_cool_ones(string):
    # by_the_way__this_greek_letters_translate_to_this_by_Python_while_reading_them_from_the_text_file = {
    #    'δ': 'Î´', 'ß': 'ÃŸ', 'µ': 'Âµ', 'ε': 'Îµ', 'Σ': 'Î£', 'Ω': 'Î©', 'Θ': 'Î˜', 'Φ': 'Î¦', '∞': 'âˆž'}

    #s peacial_char_py_converstion_dict = {'↔': 'â†”', '↨': 'â†¨', '↑': 'â†‘', '→': 'â†’', '↓': 'â†“', '‼': 'â€¼',
    #                                    '≈': 'â‰ˆ'}
    speacial_char_py_converstion_dict = {'↔': 'â†”', '↑': 'â†‘', '█': 'â–ˆ', '▒': 'â–’', '░': 'â–‘', '♣': 'â™£',
                                        '≈': 'â‰ˆ'}

    speacial_char_list = ['↔', '↑', '█', '▒', '░', '♣', '≈']
    # replace any weird characters made by python to the kewl oness::
    has_special_key_list = []
    for c in range(0, len(speacial_char_list)):
        if string.find(speacial_char_py_converstion_dict.get(speacial_char_list[c])) != -1:
            has_special_key_list += str(c)
    # checks which and how many special chars exist
    if '0' in has_special_key_list:
        newstring1 = string.replace(speacial_char_py_converstion_dict.get(speacial_char_list[0]), speacial_char_list[0])
    else: newstring1 = string
    if '1' in has_special_key_list:
        newstring2 = newstring1.replace(speacial_char_py_converstion_dict.get(speacial_char_list[1]), speacial_char_list[1])
    else:
        newstring2 = newstring1
    if '2' in has_special_key_list:
        newstring3 = newstring2.replace(speacial_char_py_converstion_dict.get(speacial_char_list[2]), speacial_char_list[2])
    else:
        newstring3 = newstring2
    if '3' in has_special_key_list:
        newstring4 = newstring3.replace(speacial_char_py_converstion_dict.get(speacial_char_list[3]), speacial_char_list[3])
    else:
        newstring4 = newstring3
    if '4' in has_special_key_list:
        newstring5 = newstring4.replace(speacial_char_py_converstion_dict.get(speacial_char_list[4]), speacial_char_list[4])
    else:
        newstring5 = newstring4
    if '5' in has_special_key_list:
        ewstring6 = newstring5.replace(speacial_char_py_converstion_dict.get(speacial_char_list[5]), speacial_char_list[5])
    else:
        ewstring6 = newstring5
    if '6' in has_special_key_list:
        newstring7 = ewstring6.replace(speacial_char_py_converstion_dict.get(speacial_char_list[6]), speacial_char_list[6])
    else:
        newstring7 = ewstring6
...
def start_program(SPEED):
    if SPEED == 3: #get speed so it affects all program.
        ...
        check_content = replace_weird_characters_with_cool_ones(check_content)
...
FunnersContent = start_program(SPEED)
...
def LoggedInProtocole(LoggedUser):
    ...
    if not '≈' in FunnersContent[0]:#Means never played Because no ≈,since ≈ happens after loging in for the 1st time
        ...
        #should look like : 'Y1█forest█forest█forest█forest...', '▒▒▒IronSword[lvl1](1){trapped}░rock▒▒
        # raw = Y0█forest█forest█forest█forest...Y20Y40≈▒▒▒IronSword[lvl1](1){trapped}░rock▒▒
    ...
    if '█' in FunnersContent_Line0_WrldTiles:
        ...
        time.sleep(1 * SPEED)
        if '▒' in FunnersContent_Line0_WrldInfo:
            FunnersContent_Line0_WrldInfo = FunnersContent_Line0_WrldInfo.split('▒')
