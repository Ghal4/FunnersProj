    ...
    if len(FunnersContent) == 0:
        ...
        if not len(FunnersContent) > 0:
            ...
            for c in range(0, len(users_list)):
                if '~' in FunnersContent[c]:
                    hasworld = 1
                else:
                    pass_to_user[users_list[c-hasworld]] = (str(FunnersContent[c])[15:23].strip())
...
commands = {
    ...
    '/funners map': 'displays the local map set on this computer... if there is one',
}
...
def close():  # /help
    ...
    if len(FunnersContent) != 0:
        refresh = open('Funners.txt', 'w')
        for c in range(0, len(FunnersContent)):
            refresh.write(str(FunnersContent[c]) + '\n')
        refresh.close()
    time.sleep(1)
...
def Funners_create():
    ...
    print('username and password should only contain numbers and/or letters + not longer than 8 characters!')
    loop = True
    while (loop):
    ...
    userpass_funner = onlyABC('\nnow set a password for your Funners'
                              '\nif you want your Funners to be public please please set your password as "PUBLIC"'
                              '\n' + yellow + '[having no password makes your Funners accessible to All!!]' + reg
                              )
    while (loop):

        if userpass_funner == 'PUBLIC' or userpass_funner == 'public' or userpass_funner == 'Public':
            ...
            for x in range((8 - len(userpass_funner))):
                userpass_funner += ' '
            loop = False
...
def LoggedInProtocole(LoggedUser):
    ...
    # find the right line.
    positionOfLoggedUserInFunnersContent = users_list.index(LoggedUser)+hasworld
    ...
    tiles_type = ['forest', 'dark forest', 'desert', 'meadows', 'swamp', 'cleft', 'lake']
    # cleft = nekik / bekkah
    biom_shapes = ['circle', 'elipse', 'big mesh <__--', 'donut', 'bites', 'corner']
    ...
    if FunnersContent_Line0 == ['WrldTiles', 'WrldInfo']: ##### generate new world!!!!!!!!!!!!!!!!!!
        ...
        temp_map = []
        for c in range(0, 20):
            temp_map.append(['forest;' * 19 + 'forest'])
        for c in range(0, len(temp_map)):
            a = ''.join(temp_map[c])
            temp_map[c] = a.split(';')

        # make everything forest ^.

        heart_tile_list = []

        def biom_to_shape_usage(biom, shape):
            the_no_no_list = [] #y0, y20, y40...
            for c in range(0, 20):
                the_no_no_list.append(20*c)

            heart_tile = random.randrange(0, 20 * 20)
            loop = True
            while (loop):
                if heart_tile in the_no_no_list or heart_tile in heart_tile_list:
                    heart_tile = random.randrange(0, 20 * 20)
                    #try again
                    loop = True
                else:
                    heart_tile_list.append(heart_tile)
                    loop = False
            # adds to the list of heart tiles in a way that it does not collide with Y's
            ...
            if shape == 'circle':
                radius_of_biom = random.randrange(3, 8)
                for c in range(0, int(radius_of_biom*radius_of_biom/2)): #number of lines to do
                    do___question_mark = True
                    try:
                        match c:
                            case 0: temp_map[y][x] = 'forest'
                            case 1: temp_map[y-1*radius_of_biom][x] = biom
                            case 2: temp_map[y+1*radius_of_biom][x] = biom
                            case 3: temp_map[y][x-1*radius_of_biom] = biom
                            case 4: temp_map[y][x+1*radius_of_biom] = biom
                            # forest  forest   forest   [#meadows#]  forest forest forest                      *
                            # forest   forest   forest      forest    forest   forest   forest
                            # [#meadows#] forest forest  [#MIDDLE#]    forest   forest [#meadows#]      *      *      *
                            # forest   forest   forest   forest      forest   forest   forest
                            # forest  forest   forest   [#meadows#] forest forest forest                       *
                            case 5: a = 1
                    except:
                        do___question_mark = False
                        # not in range of tiles
                    else:
                        match c:
                            case 0: temp_map[y][x] = 'forest'
                            case 1: temp_map[y - 1 * radius_of_biom][x] = biom
        ...
        #only for test purposes, should do random biom:
        biom_to_shape_usage('lake', 'circle')
        ...
        temp = []
        for c in range(0, len(temp_map)):
            temp.append([';'.join(temp_map[c])])
        FunnersContent_Line0[0] = temp

        print('map successfully generate!\n')
        FunnersContent[0] = str(FunnersContent_Line0[0]) + '~' + str(FunnersContent_Line0[1])
        generated_world = True

        #choose a random locaiton to start at
    ...
    WorldMapTiles = ['Y1', '0forest', '1forest', '2forest', '3forest', '4forest', '5forest', '6forest', '7forest',
                     ...
                     'Y2', '0forest', '1city wall', '2city wall', '3city wall', '4forest', '5forest', '6forest',
                     '7forest', '8forest', '9forest', '10forest', '11forest', '12forest', '13forest', '14forest',
                     '15forest', '16forest', '17forest', '18forest',
    ...
    WorldMapContent = ['', '', '',
                       ...
                       # make it so if there are humans, they should care if you take something depending on their
                       # ... position like being guards. and fight u or something
                       ]
...
def funners_display_map():
    if not '~' in FunnersContent[0]:
        print('sorry, your local file has no world set yet, you can generate one by logging in to a user!')
    else:
        try: temp_map = FunnersContent[0].split('-')
        except: temp_map = temp_map[0].split('~')
        else: temp_map = str(temp_map).split('~')
        temp_map = temp_map[0]
        temp_map = temp_map.split('[')
        temp_map = ''.join(temp_map)
        temp_map = temp_map.split(']')
        temp_map = ''.join(temp_map)
        temp_map = temp_map.split(',')
        temp_map = ''.join(temp_map)
        temp_map = temp_map.replace("' '", ';')
        temp_map = temp_map.split('"')
        temp_map = ''.join(temp_map)
        ...
        ideas = '▩▨▧▦▥▤▢□■'
        for c in range(0, len(temp_map)):
            if temp_map[c] == 'forest':
                colorfulmap = colorfulmap + green + '□ ' + reg
            elif temp_map[c] == 'lake':
                colorfulmap = colorfulmap + blue + '■ ' + reg
            #add more colors!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            if c == y:
                y = y + 21
