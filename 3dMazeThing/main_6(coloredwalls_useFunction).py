...
def startgame():
    ...
    def in_wall(pos):
        ...
        if 197 > pos[0] > 195 and 196 > pos[1] > 100:
            return [True, 'special']
        # if ...
        # if ...
        # specials ^
    ...
    def draw(AllDistances, outterwalls, specials):
        ...
        for d in range(0, len(AllDistances)):
            ...
            if d in specials:
                color = rgb_to_hex(int(255-dist*0), int(255-dist*2.5), int(255-dist*1.8))
                canvas.create_rectangle(d * 8, 0, (d + 1) * 8, rooty, fill=color)
            if d in outterwalls:
                color = rgb_to_hex(int(255-dist*1.9), int(255-dist*1.5), int(255-dist*2))
                canvas.create_rectangle(d * 8, 0, (d + 1) * 8, rooty, fill=color)
            else:
                canvas.create_rectangle(d*8, 0, (d+1)*8, rooty, fill=color)
            # drawing walls ^
            ...
            # !!!! in order to make some walls look in a different color, I have to be able to define them being special
            # that can be done by adding one more variable to [def draw], being the specific square on the screen specia
            # oh my god it worked within the first try lmao
    ...
    def calcDistance(pos, calcangle):
        ...
        for a in range(0, 100):  # how do I make this function to stop working after it has been called again?
            ...
            while(True):
                ...
                if distance == 200:
                    break
                iw = in_wall(calcPos.astype(int))
                if iw[0]: #indicates if hitting wall in general. [true/false , special trait of wall if there is]
                    if len(iw) > 1:
                        if iw[1] == 'outterwall':
                            outterwalls_on_screen.append(a)
                        if iw[1] == 'special':
                            specials_on_screen.append(a)
                    break
            AllDistances.append(distance)

        draw(AllDistances, outterwalls_on_screen, specials_on_screen)
    ...
    def use():
        if 55 > location[0].astype(int) > 45 and 55 > location[1].astype(int) > 45:
            print('GOT KEY!!!!')
        print('spcae')
    ...
    def keydown(key):
        ...
        if key.char == 'w' and not in_wall(location + velocity)[0]:
        ...
        if key.char == 'a':
            ...
            calcDistance(location, angle)
        if key.char == 's' and not in_wall(location - velocity)[0]:
            location = location - velocity
            calcDistance(location, angle)
        if key.char == 'd':
            if angle == 4*np.pi:
